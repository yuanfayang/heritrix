...
public class InvokeOperationCommandProcessor extends JmxCommandProcessor {
    ...
    public Document executeRequest(HttpInputStream in) throws IOException,
        ...
        boolean unmatchedParameters = parseArguments(in, document,
                operationElement, types, values);
    ...
    protected void invoke(Document document, Element operationElement,
            String operationVariable, List types, List values,
            ObjectName objectName) throws InstanceNotFoundException,
        ...
        if (server.isRegistered(objectName)) {
            ...
            if (!match) {
                ...
                try {
                    ...
                    Object returnValue = server.invoke(objectName,
                            operationVariable, params, signature);
                    operationElement.setAttribute("result", "success");
                    if (returnValue != null) {
                        operationElement.setAttribute("returnclass",
                                returnValue.getClass().getName());
                        operationElement.appendChild(objectToNode(document,
                                returnValue));
                    } else {
            ...
            if (objectName != null) {
                ...
                operationElement.setAttribute("errorMsg", new StringBuffer(
                        "MBean ").append(objectName).append(" not registered")
                        .toString());
            }
        }
    }

    private boolean parseArguments(HttpInputStream in, Document document,
            Element operationElement, List types, List values) {
        int i = 0;
        ...
        do {
            ...
            if (valid) {
                ...
                Object value = null;
                try {
                    value = CommandProcessorUtil.createParameterValue(
