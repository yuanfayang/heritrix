...
public class HTMLParser {
        ...
        if(this.justReadTagBegin) {
            ...
            try {
                if(tag.startsWith("!")) {
                    ...
                    Element element = doc.createElement(tag.toLowerCase());
                    ...
                    try {
                        ...
                        if (element instanceof HTMLScriptElementImpl) {
                            fillInSourceCode(element);
                        }
    ...
    private void fillInSourceCode(Element el) {
        HTMLScriptElementImpl element = (HTMLScriptElementImpl) el;
        String src = element.getSrc();
        
        if (src == null || resLocation == null) {
            return;
        }
        
        java.net.URL scriptURL = ((HTMLDocumentImpl) document).getFullURL(src);
        String urlStr = scriptURL.toString();
...
/*    private void fillInSourceCode(Element el) {
        ...
        try {
            ...
            //element.setAttribute("text", codeText);
            Node textNode = document.createTextNode(codeText);
