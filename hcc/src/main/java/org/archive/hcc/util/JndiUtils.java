...
public class JndiUtils {
    /**
     * Syntax that will work with jmx ObjectNames (i.e. will escape '.' and will
     * add treat ',' and '=' specially.
     */
    ...
    /**
     * Return name to use as jndi name. Used to do a subset of the ObjectName
     * fields but not just let all through so its easy to just use the jndi name
     * to find mbean.
     * 
     * @param on
     *            ObjectName instance to work with.
    ...
    public static Reference getReference(final ObjectName on) {
        ...
        r.add(new StringRefAddr(JmxUtils.KEY, on
     ...
     * 
     * @param context
     *            A subcontext named for the <code>on.getDomain()</code>
     ...
     * @throws NullPointerException
     */
    public static CompoundName bindObjectName(
            Context context,
            final ObjectName on) throws NamingException, NullPointerException {
        ...
        //
        // String name =
        // getCompoundName(context.getNameInNamespace()).toString();
    ...
    public static CompoundName unbindObjectName(
            final Context context,
            final ObjectName on) throws NullPointerException, NamingException {
        CompoundName key = getCompoundName(on);
        context.unbind(key);
        return key;
    }
