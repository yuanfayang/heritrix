...
<%
	...
	if(request.getParameter(handler.XP_CRAWL_ORDER_NAME) != null)
	{
		...
		String filename = ArchiveUtils.getFilePath(job.getCrawlOrderFile())+"job-"+request.getParameter(handler.XP_CRAWL_ORDER_NAME)+"-"+(job.getOrderVersion()+1)+".xml";
		handler.createCrawlOrderFile(request,filename,"seeds-"+request.getParameter(handler.XP_CRAWL_ORDER_NAME)+".txt",true);
	...
	<% } else { %>
		...
		<table border="0">
			...
			<tr>
				...
				<td>
					<textarea name="<%=handler.XP_SEEDS%>" rows="8" cols="<%=iInputSize%>"><%
