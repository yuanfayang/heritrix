 ...
 * 
 * A peek() guarantees that the item returned by peek() will be returned
 * by the next dequeue(), even if other items are enqueued to lower-numbered
 * internal queues in the meantime. (The unpeek() may be used to release
 * the TieredQueue from this guarantee; the next peek()/dequeue() will then 
 * return the item available from the lowest-numbered queue.)
 * 
 * @author gojomo
 */
public class TieredQueue implements Queue {
    ...
    public Object dequeue() throws NoSuchElementException {
        Object o = peek();
        assert (o == innerQueues[headSource].dequeue()) : "TieredQueue innerQueue[headSource] != headObject";
        headObject = null;
        length--;
        return o;
    }
