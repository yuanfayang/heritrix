 ...
 */
public class LegacyExtractorSWF extends Extractor implements CoreAttributeConstants {

	private static final long serialVersionUID = 3627359592408010589L;

	private static Logger logger = Logger.getLogger(LegacyExtractorSWF.class
			.getName());
	 ...
	 * @param name
	 */
	public LegacyExtractorSWF(String name) {
		...
		SWFReader reader = new SWFReader(getTagParser(tags), documentStream) {
			...
			public int readOneTag() throws IOException {
				...
				if (length > MAX_READ_SIZE) {
					...
					logger.fine("calling mConsumer.tag() type=" + type + " longTag=" + longTag + " contents=" + contents);
	...
	private TagParser getTagParser(SWFTagTypes customTags) {
		logger.fine("customTags=" + customTags);
	...
	protected class ExtractorSWFTags extends SWFTagTypesImpl {
		...
		public ExtractorSWFTags(SWFActions acts) {
			super(null);
			actions = acts;
			logger.fine("acts=" + acts);
		}

		public SWFActions tagDefineButton(int id, Vector buttonRecords)
				throws IOException {
		    logger.fine("tag: DefineButton: looking for links to extract in actions");
			return actions;
		}

		public SWFActions tagDefineButton2(int id, boolean trackAsMenu,
				Vector buttonRecord2s) throws IOException {
            logger.fine("tag: DefineButton2: looking for links to extract in actions");
			return actions;
		}

		public SWFActions tagDoAction() throws IOException {
            logger.fine("tag: DoAction: looking for links to extract in actions");
			return actions;
		}

		public SWFActions tagDoInActions(int spriteId) throws IOException {
            logger.fine("tag: DoInActions: looking for links to extract in actions");
			return actions;
		}

		public SWFTagTypes tagDefineSprite(int id) throws IOException {
            logger.fine("tag: DefineSprite: looking for links to extract in sub-tags");
			return this;
		}
		...
		public SWFActions tagPlaceObject2(boolean isMove, int clipDepth,
		    ...
		    logger.fine("tag: PlaceObject2: looking for links to extract in actions");
		    return actions;
		}
	...
	protected class ExtractorSWFActions extends SWFActionsImpl {
		...
		public ExtractorSWFActions(CrawlURI curi, CrawlController controller) {
			...
			logger.fine("curi=" + curi + " controller=" + controller);
		}
		...
		public void considerStringAsUri(String str) throws IOException {
			...
			if (uri.matches()) {
			    logger.fine("found link: " + uri.group());
