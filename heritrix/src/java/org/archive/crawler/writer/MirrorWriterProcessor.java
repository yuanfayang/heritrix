...
extends Processor implements CoreAttributeConstants {
     ...
     */
    public static final String ATTR_CASE_SENSITIVE = "case-sensitive";
     ...
     */
    public static final String ATTR_CONTENT_TYPE_MAP = "content-type-map";

    /**
     * Key to use asking settings for dot begin replacement.
     */
    public static final String ATTR_DOT_BEGIN = "dot-begin";

    /**
     * Key to use asking settings for dot end replacement.
     */
    public static final String ATTR_DOT_END = "dot-end";

    /**
     * Key to use asking settings for directory file.
     */
    public static final String ATTR_DIRECTORY_FILE = "directory-file";

    /**
     * Key to use asking settings for host directory option.
     */
    public static final String ATTR_HOST_DIRECTORY = "host-directory";

    /**
     * Key to use asking settings for host map.
     */
    public static final String ATTR_HOST_MAP = "host-map";

    /**
     * Key to use asking settings for maximum file system path length.
     */
    public static final String ATTR_MAX_PATH_LEN = "max-path-length";

    /**
     * Key to use asking settings for maximum file system path segment length.
     */
    public static final String ATTR_MAX_SEG_LEN = "max-segment-length";
     ...
     */
    public static final String ATTR_PORT_DIRECTORY = "port-directory";

    /**
     * Key to use asking settings for suffix at end option.
     */
    public static final String ATTR_SUFFIX_AT_END = "suffix-at-end";

    /**
     * Key to use asking settings for too-long directory.
     */
    public static final String ATTR_TOO_LONG_DIRECTORY = "too-long-directory";

    /**
     * Key to use asking settings for underscore set.
     */
    public static final String ATTR_UNDERSCORE_SET = "underscore-set";

    /** Default value for ATTR_DOT_BEGIN.*/
    private static final String DEFAULT_DOT_BEGIN = "%2E";

    /** Default maximum file system path length.*/
    private static final int DEFAULT_MAX_PATH_LEN = 1023;

    /** Default maximum file system path segment length.*/
    private static final int DEFAULT_MAX_SEG_LEN = 255;
    ...
    /**
       ...
       The intent is one or more non-file-separator characters.
       The backslash is to quote File.separator if it's also backslash.
    */
    private static final String PATH_SEGMENT_RE =
        "[^\\" + File.separator + "]+";

    /**
       Regular expression constraint on ATTR_DIRECTORY_FILE.
       The intent is one non-file-separator character,
       followed by zero or more characters.
       The backslash is to quote File.separator if it's also backslash.
    */
    private static final String TOO_LONG_DIRECTORY_RE =
        "[^\\" + File.separator + "].*";
    ...
    public MirrorWriterProcessor(String name) {
        ...
        e = addElementToDefinition(new SimpleType(ATTR_DIRECTORY_FILE,
            ...
            "index.html"));
        e.addConstraint(new RegularExpressionConstraint(PATH_SEGMENT_RE,
            Level.SEVERE, "This must be a simple file name."));
        e = addElementToDefinition(new SimpleType(ATTR_DOT_BEGIN,
            "If a segment starts with '.', the '.' is replaced by this.",
            DEFAULT_DOT_BEGIN));
        e.addConstraint(new RegularExpressionConstraint(PATH_SEGMENT_RE,
            Level.SEVERE,
            "This must not be empty, and must not contain " + File.separator));
        ...
        addElementToDefinition(new SimpleType(ATTR_MAX_PATH_LEN,
            ...
            new Integer(DEFAULT_MAX_PATH_LEN)));
        addElementToDefinition(new SimpleType(ATTR_MAX_SEG_LEN,
            "Maximum file system path segment length.",
            new Integer(DEFAULT_MAX_SEG_LEN)));
        addElementToDefinition(new SimpleType(ATTR_PORT_DIRECTORY,
        ...
        e.addConstraint(new RegularExpressionConstraint(TOO_LONG_DIRECTORY_RE,
            Level.SEVERE, "This must be relative and not empty."));
    ...
    */
    private URIToFileReturn uriToFile(String baseDir, CrawlURI curi)
        ...
        UURI uuri = curi.getUURI(); // Current URI.
        String host = null;
        Boolean hd = (Boolean) getAttribute(ATTR_HOST_DIRECTORY, curi);
        if (hd.booleanValue()) {
            host = uuri.getHost();
        ...
        int port =
            ((Boolean) getAttribute(ATTR_PORT_DIRECTORY, curi)).booleanValue()
            ? uuri.getPort()
            : -1;
        ...
        int maxSegLen =
            ((Integer) getAttribute(ATTR_MAX_SEG_LEN, curi)).intValue();
        if (maxSegLen < 2) {
            maxSegLen = DEFAULT_MAX_SEG_LEN;
        }

        int maxPathLen =
            ((Integer) getAttribute(ATTR_MAX_PATH_LEN, curi)).intValue();
        if (maxPathLen < 2) {
        ...
        String dotBegin = (String) getAttribute(ATTR_DOT_BEGIN, curi);
        if (".".equals(dotBegin)) {
            dotBegin = null;
        }

        String dotEnd = (String) getAttribute(ATTR_DOT_END, curi);
        if (".".equals(dotEnd)) {
            dotEnd = null;
        }

        String tld = (String) getAttribute(ATTR_TOO_LONG_DIRECTORY, curi);
        ...
        return uriToFile(curi, host, port, uuri.getPath(), uuri.getQuery(),
            ...
            ((Boolean) getAttribute(ATTR_CASE_SENSITIVE, curi)).booleanValue(),
            (String) getAttribute(ATTR_DIRECTORY_FILE, curi),
            characterMap, dotBegin, dotEnd, tld,
            ((Boolean) getAttribute(ATTR_SUFFIX_AT_END, curi)).booleanValue(),
            underscoreSet);
    }
