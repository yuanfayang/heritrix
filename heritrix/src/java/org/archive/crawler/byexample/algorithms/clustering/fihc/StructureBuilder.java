...
public class StructureBuilder {
    ...
    /**
     * Disjoin clusters - after this method is run each document belongs to
     * exactly one cluster
     * 
     */
    public void disjoinClusters() {
        ...
        for (Iterator<ItemSet> iter = myDocumentClusteringIndex
            ...
            for (int i = 0; i < currRow.length; i++) {
                currDocID = currRow[i];
                currScore = ScoreComputation.calculateClusterDocumentScore(
                        currIS, currDocID, myClusterSupportIndex, myTFIDFIndex,
                        myGlobalSupportIndex);
                // Initialize best cluster and score if this is the first time
                // document is encountered
                if (clusterScoresHash.get(currDocID) == null) {
                    clusterScoresHash.put(currDocID, new ClusterScore(currIS,
                            currScore));
                    continue;
                }
                // Update best cluster and score and remove document from
                // previously best cluster
                if (currScore >= clusterScoresHash.get(currDocID)
                        .getClusterScore()) {
                    myDocumentClusteringIndex.getRow(
                            clusterScoresHash.get(currDocID).getClusterLabel())
                            .removeValue(currDocID);
                    clusterScoresHash.put(currDocID, new ClusterScore(currIS,
                            currScore));
                }
                else
    ...
    //Calculate cluster support for a given cluster
    private void calculateClusterSupport(ItemSet currIS){
        IdListing currDocs = null;
        int currDocsNo = 0;
        TermSupport currSupportEntry;
        TermSupportListing currSupportRow=null;
        ...
        currSupportRow=myClusterSupportIndex.getRow(currIS);
        for (int i = 0; i < myGlobalSupportIndex.size(); i++) {
            currSupportEntry=currSupportRow.getEntryAtIndex(i);
            //Sum up all cluster documents supports
            for (int j = 0; j < currDocsNo; j++) {
                currSupportEntry.setSupport(
                                (myDocumentSupportIndex.getRow(
                                        currDocs.getValueAtPos(j))
                                        .getEntryAtIndex(i).getSupport() + 
                                        currSupportEntry.getSupport()));
            }
            //Normalize
            currSupportEntry.setSupport(currSupportEntry.getSupport()/currDocsNo);
        }
    }
    ...
    public void calculateDocumentSupport() {
        ...
        for (Iterator<DocumentEntry> iter = myDocListing.getListingIterator(); iter
                .hasNext();) {
            currDocId = String.valueOf(iter.next().getId());
            myDocumentSupportIndex.initializeKey(currDocId,
                    myGlobalSupportIndex);
            for (int i = 0; i < myGlobalSupportIndex.size(); i++) {
                currSE = myDocumentSupportIndex.getRow(currDocId)
                        .getEntryAtIndex(i);
                currIR = myTermsIndex.getIndex().getRow(
                        myGlobalSupportIndex.get(i).getTerm());
    ...
    /**
     * Creates initial clustering based on given FrequentItemSets. At this
     * point, each document can belong to several clusters.
     */
    public void createInitialDocumentClusters(FrequentItemSets fis) {
        ...
        ItemSet unClassified = new ItemSet(
                ByExampleProperties.UNCLASSIFIED_LABEL);

        for (Iterator<DocumentEntry> docIter = myDocListing
                .getListingIterator(); docIter.hasNext();) {
            isClassified = false;
            docId = String.valueOf(docIter.next().getId());

            for (Iterator<ItemSet> itemIter = fis.getSetsIterator(); itemIter
                    .hasNext();) {
            ...
            // Gather all the documents that hasn't been classified under
            // special UNCLASSIFIED cluster
            if (!isClassified)
    ...
    /**
     * Returns true if document contains all items in given ItemSet. Otherwise,
     * returns false
     */
    public boolean containsItemSet(ItemSet is, String docId) {
        String[] items = is.getItems();
        for (String currItem : items) {
            // Binary search can be employed, since terms list is sorted
            // lexicograhically
    ...
    /**
     * Builds clustering structure based on given FrequentItemSets and reports
     * invocation times for each step This is the only method that should be
     * invoked by outside classes
     * 
     * @param fis
     *            given FrequentItemSets
     */
